...
FACE_MODEL = 'Facenet512'  # Pode usar: VGG-Face, Facenet, Facenet512, OpenFace, DeepFace, DeepID, ArcFace, Dlib, SFace
DETECTOR_BACKEND = 'opencv'  # Pode usar: opencv, ssd, dlib, mtcnn, retinaface, mediapipe
...
def extract_face_embedding(image):
    ...
    try:
        ...
        # DeepFace retorna uma lista de dicionários, pegamos o primeiro embedding
        embedding = np.array(embedding_objs[0]['embedding'])

        return embedding, None

    except ValueError as ve:
        # Erro quando nenhuma face é detectada
        return None, "Nenhuma face detectada na imagem"
    except Exception as e:
...
def calculate_similarity(embedding1, embedding2):
    """Calcula a similaridade entre dois embeddings usando distância euclidiana normalizada"""
    ...
    # Normaliza para similaridade (quanto menor a distância, maior a similaridade)
    # Para Facenet512, distâncias típicas são entre 0 e 40
    # Convertemos para escala 0-1 onde 1 é idêntico
...
def verify_face():
    ...
    try:
        ...
        try:
            ...
            # Threshold ajustado para DeepFace com Facenet512
            DISTANCE_THRESHOLD = 10
            SIMILARITY_THRESHOLD = 0.6

            for colaborador in colaboradores:
                stored_embedding = np.array(json.loads(colaborador['facial_embedding']))
                similarity, distance = calculate_similarity(current_embedding, stored_embedding)
...
if __name__ == '__main__':
    print(f"Iniciando API Facial com DeepFace")
    print(f"Modelo: {FACE_MODEL}")
